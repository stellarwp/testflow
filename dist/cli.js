...
class LandoManager {
  ...
  wordpressConfig;
  landoFile = ".lando.yml";
  constructor(config, sqlConfig, wordpressConfig) {
    this.config = config;
    this.sqlConfig = sqlConfig;
    this.wordpressConfig = wordpressConfig;
  }
  async setup() {
    console.log(source_default.blue("\uD83D\uDE80 Setting up Lando environment..."));
    ...
    console.log(source_default.green("✅ Lando environment ready"));
  }
  async setupWordPressFromSql() {
    console.log(source_default.blue("\uD83D\uDCC1 Setting up WordPress from SQL files..."));
    if (!this.sqlConfig?.files || this.sqlConfig.files.length === 0) {
      throw new Error("No SQL files specified for WordPress setup");
    }
    try {
      console.log(source_default.gray("Creating WordPress file structure..."));
      execSync("lando wp core download --skip-content", { stdio: "inherit" });
      console.log(source_default.gray("Creating wp-config.php..."));
      ...
      console.log(source_default.green("✅ WordPress setup from SQL completed"));
    } catch (error) {
      console.error(source_default.red(`❌ Failed to setup WordPress from SQL: ${error}`));
      if (!this.sqlConfig?.continueOnError) {
        throw error;
      }
    }
  }
  async performUrlSearchReplace() {
    if (!this.sqlConfig?.searchReplace?.enabled && !this.sqlConfig?.insteadOfWordPress) {
      return;
    }
    console.log(source_default.blue("\uD83D\uDD04 Performing URL search and replace..."));
    try {
      ...
      if (this.sqlConfig?.searchReplace?.fromUrl) {
        ...
        console.log(source_default.gray(`Replacing ${fromUrl} with ${toUrl}...`));
        ...
        console.log(source_default.gray("Auto-detecting URLs for replacement..."));
        const currentUrlCmd = 'lando wp option get siteurl 2>/dev/null || echo ""';
        let currentUrl = "";
        try {
          currentUrl = execSync(currentUrlCmd, { encoding: "utf8" }).trim();
        } catch (error) {
          console.log(source_default.yellow("Could not detect current site URL, skipping auto-replace"));
        }
        if (currentUrl && currentUrl !== siteUrl) {
          console.log(source_default.gray(`Auto-replacing ${currentUrl} with ${siteUrl}...`));
        ...
        try {
          ...
          if (currentHome && currentHome !== siteUrl && currentHome !== currentUrl) {
            console.log(source_default.gray(`Auto-replacing home URL ${currentHome} with ${siteUrl}...`));
      ...
      if (this.sqlConfig?.searchReplace?.additionalReplacements) {
        for (const replacement of this.sqlConfig.searchReplace.additionalReplacements) {
          console.log(source_default.gray(`Replacing ${replacement.from} with ${replacement.to}...`));
      ...
      console.log(source_default.green("✅ URL search and replace completed"));
    } catch (error) {
      console.error(source_default.red(`❌ URL search and replace failed: ${error}`));
      if (!this.sqlConfig?.continueOnError) {
        throw error;
      }
    }
  ...
  async updateAdminCredentials() {
    ...
    console.log(source_default.blue("\uD83D\uDC64 Updating admin user credentials..."));
    try {
      ...
      try {
        ...
        console.log(source_default.gray(`Updated existing user: ${adminUser}`));
      } catch (error) {
        console.log(source_default.gray(`Creating admin user: ${adminUser}`));
        execSync(`lando wp user create "${adminUser}" "${adminEmail}" --role=administrator --user_pass="${adminPassword}"`, { stdio: "inherit" });
      }
      console.log(source_default.green("✅ Admin credentials updated"));
    } catch (error) {
      console.error(source_default.red(`❌ Failed to update admin credentials: ${error}`));
      if (!this.sqlConfig?.continueOnError) {
        throw error;
      }
    }
  ...
  async setupWordPress() {
    console.log(source_default.blue("\uD83D\uDD27 Installing WordPress..."));
    try {
      ...
      console.log(source_default.green("✅ WordPress installation completed"));
    } catch (error) {
      console.error(source_default.red(`❌ WordPress installation failed: ${error}`));
      throw error;
    }
  }
  async executeSqlFiles(stage, files) {
    if (!files || files.length === 0) {
      return;
    }
    console.log(source_default.blue(`\uD83D\uDCC4 Executing SQL files at stage: ${stage}`));
    for (const file of files) {
      ...
      try {
        console.log(source_default.gray(`Executing: ${file}`));
        const sqlContent = readFileSync(file, "utf8");
        execSync(`lando mysql wordpress < "${file}"`, { stdio: "inherit" });
        console.log(source_default.green(`✅ Executed: ${file}`));
      } catch (error) {
...
tooling:
  ...
  mysql:
    service: database
`);
    console.log(source_default.gray("Generated .lando.yml"));
  }
  async startLando() {
    console.log(source_default.blue("\uD83D\uDD04 Starting Lando..."));
    try {
      execSync("lando start", { stdio: "inherit" });
      console.log(source_default.green("✅ Lando started"));
    } catch (error) {
      console.error(source_default.red(`❌ Failed to start Lando: ${error}`));
      throw error;
    }
  }
  async stop() {
    console.log(source_default.blue("\uD83D\uDED1 Stopping Lando..."));
    try {
      execSync("lando stop", { stdio: "inherit" });
      console.log(source_default.green("✅ Lando stopped"));
    } catch (error) {
      console.error(source_default.red(`❌ Failed to stop Lando: ${error}`));
      throw error;
    }
  }
  async destroy() {
    console.log(source_default.blue("\uD83D\uDCA5 Destroying Lando environment..."));
    try {
      execSync("lando destroy -y", { stdio: "inherit" });
      console.log(source_default.green("✅ Lando environment destroyed"));
    } catch (error) {
      console.error(source_default.red(`❌ Failed to destroy Lando: ${error}`));
      throw error;
    }
  }
}
...
class PluginManager {
  ...
  async resolvePluginZips() {
    ...
    for (const pattern of this.config.zips) {
      ...
      for (const match2 of matches) {
        if (match2.endsWith(".zip") && existsSync2(match2)) {
          resolvedPaths.push(match2);
        }
      }
    }
  ...
  async installPlugin(zipPath) {
    ...
    try {
      if (!existsSync2(this.tempDir)) {
        mkdirSync(this.tempDir, { recursive: true });
      }
      await import_extract_zip.extract(zipPath, { dir: join(process.cwd(), tempExtractPath) });
      ...
      try {
        execSync2(`lando ${installCommand}`, { stdio: "pipe" });
      } catch (error) {
        await this.copyPluginDirectory(pluginDir, pluginName);
      }
      this.installedPlugins.push(pluginName);
      console.log(source_default.green(`✅ Plugin installed: ${pluginName}`));
    } catch (error) {
  ...
  async activatePlugins() {
    if (!this.config.preActivate && this.config.activateList.length === 0) {
      console.log(source_default.yellow("⏭️  Plugin activation skipped"));
      return;
    }
    let pluginsToActivate = [];
    if (this.config.preActivate) {
      pluginsToActivate = [...this.installedPlugins];
    }
    pluginsToActivate.push(...this.config.activateList);
    pluginsToActivate = pluginsToActivate.filter((plugin) => !this.config.skipActivation.includes(plugin));
    pluginsToActivate = [...new Set(pluginsToActivate)];
    if (pluginsToActivate.length === 0) {
      console.log(source_default.yellow("⏭️  No plugins to activate"));
      return;
    }
    try {
  ...
  async activatePlugin(pluginName) {
    try {
      execSync2(`lando wp plugin activate ${pluginName}`, { stdio: "pipe" });
      console.log(source_default.green(`✅ Plugin activated: ${pluginName}`));
    } catch (error) {
      console.warn(source_default.yellow(`⚠️  Failed to activate plugin: ${pluginName}`));
      const pluginFiles = await this.findPluginFiles(pluginName);
      for (const file of pluginFiles) {
        try {
          execSync2(`lando wp plugin activate ${file}`, { stdio: "pipe" });
          console.log(source_default.green(`✅ Plugin activated: ${file}`));
          return;
        } catch {}
      }
      console.warn(source_default.yellow(`⚠️  Could not activate plugin: ${pluginName}`));
    }
  }
  async deactivatePlugin(pluginName) {
    try {
      execSync2(`lando wp plugin deactivate ${pluginName}`, { stdio: "pipe" });
      console.log(source_default.green(`✅ Plugin deactivated: ${pluginName}`));
    } catch (error) {
      console.warn(source_default.yellow(`⚠️  Failed to deactivate plugin: ${pluginName}`));
    }
  }
  async activateSpecificPlugins(pluginNames) {
    for (const pluginName of pluginNames) {
      await this.activatePlugin(pluginName);
    }
  }
  async deactivateSpecificPlugins(pluginNames) {
    for (const pluginName of pluginNames) {
  ...
  async getInstalledPlugins() {
    try {
      const output = execSync2("lando wp plugin list --format=json", {
        encoding: "utf8",
        stdio: "pipe"
      });
      ...
      console.warn(source_default.yellow("Failed to get installed plugins list"));
      return [];
    }
  }
  async getActivePlugins() {
    try {
      const output = execSync2("lando wp plugin list --status=active --format=json", {
        encoding: "utf8",
        stdio: "pipe"
      });
      const plugins = JSON.parse(output);
      ...
      console.warn(source_default.yellow("Failed to get active plugins list"));
      return [];
    }
  }
  async isPluginActive(pluginName) {
    const activePlugins = await this.getActivePlugins();
    return activePlugins.some((plugin) => plugin.includes(pluginName) || pluginName.includes(plugin));
  }
  async cleanup() {
    if (existsSync2(this.tempDir)) {
      try {
        ...
        console.warn(source_default.yellow(`Warning: Failed to cleanup temp directory: ${error}`));
      }
    }
  }
  async copyPluginDirectory(sourceDir, pluginName) {
    try {
      const targetDir = `wp-content/plugins/${pluginName}`;
      execSync2(`lando mkdir -p ${targetDir}`, { stdio: "pipe" });
      execSync2(`lando cp -r ${sourceDir}/* ${targetDir}/`, { stdio: "pipe" });
      execSync2(`lando chown -R www-data:www-data ${targetDir}`, { stdio: "pipe" });
      execSync2(`lando chmod -R 755 ${targetDir}`, { stdio: "pipe" });
    } catch (error) {
      throw new Error(`Failed to copy plugin directory: ${error}`);
    }
  }
  async findPluginFiles(pluginName) {
    try {
      const output = execSync2(`lando wp plugin list --format=json`, {
        encoding: "utf8",
        stdio: "pipe"
      ...
      const plugins = JSON.parse(output);
      const matchingPlugins = plugins.filter((plugin) => plugin.name.includes(pluginName) || plugin.file.includes(pluginName));
      return matchingPlugins.map((plugin) => plugin.file);
    } catch (error) {
      return [];
    }
  }
...
class PlaywrightManager {
  ...
  async discoverTests() {
    ...
    for (const pattern of patterns) {
      const searchPath = join2(this.config.testDir, pattern);
      const matches = await glob(searchPath, {
        absolute: true,
        ignore: ["**/node_modules/**", "**/.git/**"]
      });
      for (const match2 of matches) {
        if (existsSync3(match2)) {
          testFiles.push(match2);
        }
      }
    }
  ...
  async runTests(testFiles = []) {
    ...
    try {
      ...
      const output = execSync3(playwrightCmd, {
        encoding: "utf8",
        stdio: "pipe"
      });
      ...
      if (this.options.debug) {
        console.log(source_default.blue("\uD83D\uDCCA Test Results:"));
        console.log(source_default.green(`  ✅ Passed: ${results.passed}`));
        console.log(source_default.red(`  ❌ Failed: ${results.failed}`));
        console.log(source_default.yellow(`  ⏭️  Skipped: ${results.skipped}`));
        console.log(source_default.blue(`  \uD83D\uDCCB Total: ${results.total}`));
      }
      return results;
    } catch (error) {
      ...
      if (errorOutput.includes("failed")) {
        ...
        results.total = testFiles.length;
        if (this.options.debug) {
          console.log(source_default.red("❌ Test execution failed:"));
      ...
      projects: [
        {
          name: "chromium",
          use: {
            ...__require("@playwright/test").devices["Desktop Chrome"],
...
`;
  ...
  buildPlaywrightCommand(testFiles) {
  ...
  parseTestResults(output, type) {
    ...
    const match2 = output.match(pattern);
...
class TestFlowRunner {
  ...
  constructor(config, options = {}) {
    ...
    this.landoManager = new LandoManager(this.config.lando, this.config.sql, this.config.wordpress);
  ...
  async runWithMatrix() {
    ...
    console.log(source_default.blue(`\uD83D\uDD04 Running ${matrixConfigurations.length} matrix configurations...`));
...
\uD83D\uDCCB Matrix ${i + 1}/${matrixConfigurations.length}: ${matrixConfig.name}`));
      if (matrixConfig.sqlFiles && matrixConfig.sqlFiles.length > 0) {
        console.log(source_default.gray(`   SQL Files: ${matrixConfig.sqlFiles.join(", ")}`));
      }
      if (matrixConfig.plugins && matrixConfig.plugins.length > 0) {
        console.log(source_default.gray(`   Plugins: ${matrixConfig.plugins.join(", ")}`));
      }
      if (matrixConfig.environment) {
        console.log(source_default.gray(`   Environment: ${matrixConfig.environment}`));
      }
      try {
        ...
        console.error(source_default.red(`❌ Matrix configuration ${i + 1} failed: ${error}`));
        results.push({
          name: matrixConfig.name,
          result: {
            passed: 0,
  ...
  generateMatrixConfigurations() {
    ...
    if (this.config.matrix.environments) {
      for (const env2 of this.config.matrix.environments) {
        const config = { ...this.config };
        if (env2.php)
          config.lando.php = env2.php;
        if (env2.mysql)
          config.lando.mysql = env2.mysql;
        if (env2.wordpress)
          config.lando.wordpress = env2.wordpress;
        if (env2.sql_files) {
          config.sql = {
            ...config.sql,
            files: env2.sql_files,
            insteadOfWordPress: env2.insteadOfWordPress || false
          };
          if (env2.searchReplace) {
            config.sql.searchReplace = {
              ...config.sql.searchReplace,
              enabled: true,
              fromUrl: env2.searchReplace.fromUrl,
              toUrl: env2.searchReplace.toUrl || config.wordpress.siteUrl
            };
          }
        }
        if (env2.plugins) {
          config.plugins = {
            ...config.plugins,
            activateList: env2.plugins
          };
        }
        configurations.push({
          name: env2.name,
          config,
          sqlFiles: env2.sql_files,
          plugins: env2.plugins,
          environment: env2.name,
          insteadOfWordPress: env2.insteadOfWordPress
        });
      }
    }
  ...
  generateMatrixName(sqlFiles, plugins) {
    const parts = [];
    if (sqlFiles.length > 0) {
...
\uD83D\uDCCA Matrix Test Summary:`));
    ...
    for (const result of results) {
      const status = result.result.failed > 0 ? source_default.red("❌ FAILED") : source_default.green("✅ PASSED");
      console.log(`${status} ${result.name}`);
      console.log(source_default.gray(`   Passed: ${result.result.passed}, Failed: ${result.result.failed}, Skipped: ${result.result.skipped}`));
      if (result.result.errors && result.result.errors.length > 0) {
        console.log(source_default.red(`   Errors: ${result.result.errors.join(", ")}`));
      }
    ...
    console.log(source_default.gray("═".repeat(80)));
    console.log(source_default.blue(`Total: ${totalPassed} passed, ${totalFailed} failed, ${totalSkipped} skipped`));
    if (totalFailed > 0) {
...
✅ All ${results.length} matrix configurations passed`));
  ...
  async cleanup() {
    try {
      ...
      console.warn(source_default.yellow(`Warning: Cleanup failed: ${error}`));
    }
  }
  async destroy() {
    try {
      ...
      console.warn(source_default.yellow(`Warning: Destroy failed: ${error}`));
    }
  }
}
...
class ConfigManager {
  configPath;
  constructor(configPath = "testflow.yaml") {
    this.configPath = configPath;
  }
  async loadConfig(configPath, profile) {
  ...
  async initConfig(options = {}) {
    ...
    writeFileSync2(configPath, yamlContent, "utf8");
    console.log(source_default.green(`✅ Configuration initialized: ${configPath}`));
  }
  createSingleConfig(options) {
    const config = {
  ...
  createMultiProfileConfig(options) {
  ...
  async listProfiles() {
    ...
    try {
      const content = readFileSync2(this.configPath, "utf8");
      ...
      console.warn(source_default.yellow(`Failed to list profiles: ${error}`));
      return [];
    }
  }
  async validateConfig(config) {
    const errors2 = [];
    if (!config.lando) {
      errors2.push("Missing lando configuration");
    } else {
      if (!config.lando.php)
        errors2.push("lando.php is required");
      if (!config.lando.mysql)
        errors2.push("lando.mysql is required");
      if (!config.lando.wordpress)
        errors2.push("lando.wordpress is required");
    }
    if (!config.playwright) {
      errors2.push("Missing playwright configuration");
    } else {
      if (!config.playwright.testDir)
        errors2.push("playwright.testDir is required");
    }
    if (!config.plugins) {
      errors2.push("Missing plugins configuration");
    } else {
      if (!config.plugins.zips || config.plugins.zips.length === 0) {
        errors2.push("plugins.zips must contain at least one ZIP file pattern");
      }
    }
    if (errors2.length > 0) {
...
program2.name("testflow").description("WordPress Plugin Testing Framework with Lando and Playwright").version("1.0.0");
program2.command("run").description("Run TestFlow tests").option("-c, --config <path>", "Configuration file path", "testflow.yaml").option("-p, --profile <name>", "Configuration profile to use").option("--debug", "Enable debug mode").option("--matrix", "Run matrix tests (all combinations)").option("--matrix-index <index>", "Run specific matrix configuration (0-based index)", parseInt).option("--sql-files <files>", "Comma-separated list of SQL files to execute").option("--sql-stage <stage>", "SQL execution stage", "after-wordpress").option("--sql-continue-on-error", "Continue execution if SQL files fail").option("--instead-of-wordpress", "Use SQL files instead of WordPress installation").option("--search-replace-from <url>", "URL to replace from SQL").option("--search-replace-to <url>", "URL to replace to").action(async (options) => {
  try {
    const config = await loadConfig(options.config, options.profile);
    if (options.sqlFiles) {
      config.sql = {
        ...config.sql,
        files: options.sqlFiles.split(",").map((f) => f.trim())
      };
    }
    if (options.sqlStage) {
      config.sql = {
        ...config.sql,
        executeOrder: options.sqlStage
      };
    }
...
var matrixCmd = program2.command("matrix").description("Matrix testing commands");
matrixCmd.command("list").description("List available matrix configurations").option("-c, --config <path>", "Configuration file path", "testflow.yaml").option("-p, --profile <name>", "Configuration profile to use").action(async (options) => {
  try {
    const config = await loadConfig(options.config, options.profile);
    if (!config.matrix) {
      console.log(source_default.yellow("⚠️  No matrix configuration found"));
      return;
    }
    console.log(source_default.blue("\uD83D\uDCCB Available Matrix Configurations:"));
    console.log(source_default.gray("═".repeat(60)));
    if (config.matrix.sql_files) {
      console.log(source_default.green(`
\uD83D\uDDC4️  SQL Files Matrix:`));
      config.matrix.sql_files.forEach((files, index) => {
        console.log(source_default.gray(`   ${index}: ${files.join(", ")}`));
      });
    }
...
\uD83D\uDD0C Plugin Combinations Matrix:`));
      config.matrix.plugin_combinations.forEach((plugins, index) => {
        console.log(source_default.gray(`   ${index}: ${plugins.join(", ")}`));
      });
    }
    if (config.matrix.environments) {
      console.log(source_default.green(`
\uD83C\uDF0D Environments Matrix:`));
      config.matrix.environments.forEach((env2, index) => {
        console.log(source_default.gray(`   ${index}: ${env2.name}`));
        if (env2.sql_files) {
          console.log(source_default.gray(`      SQL: ${env2.sql_files.join(", ")}`));
        }
        if (env2.plugins) {
          console.log(source_default.gray(`      Plugins: ${env2.plugins.join(", ")}`));
        }
        if (env2.insteadOfWordPress) {
          console.log(source_default.gray(`      Instead of WordPress: ${env2.insteadOfWordPress}`));
        }
      });
    }
  } catch (error) {
...
var sqlCmd = program2.command("sql").description("SQL file management commands");
sqlCmd.command("execute <files...>").description("Execute SQL files directly").option("-c, --config <path>", "Configuration file path", "testflow.yaml").option("-p, --profile <name>", "Configuration profile to use").option("--continue-on-error", "Continue execution if SQL files fail").option("--instead-of-wordpress", "Use SQL files instead of WordPress installation").option("--search-replace-from <url>", "URL to replace from SQL").option("--search-replace-to <url>", "URL to replace to").action(async (files, options) => {
  try {
    const config = await loadConfig(options.config, options.profile);
    config.sql = {
      ...config.sql,
    ...
    if (options.insteadOfWordpress) {
      ...
      console.log(source_default.blue("\uD83D\uDCC4 Executing SQL files..."));
      await landoManager.executeSqlFiles("manual-execution", files);
      console.log(source_default.green("✅ SQL files executed"));
    }
  } catch (error) {
    console.error(source_default.red(`❌ Error: ${error}`));
    process.exit(1);
...
program2.command("init").description("Initialize TestFlow configuration").option("--name <name>", "Project name").option("--multi-profile", "Generate multi-profile configuration").option("--with-sql", "Include SQL configuration examples").option("--with-matrix", "Include matrix testing configuration").option("--php <version>", "PHP version", "8.2").option("--mysql <version>", "MySQL version", "8.0").option("--wordpress <version>", "WordPress version", "6.4").action(async (options) => {
  try {
    ...
    console.log(source_default.green("✅ TestFlow configuration initialized"));
    if (options.withSql || options.withMatrix) {
      console.log(source_default.blue(`
\uD83D\uDCD6 Next steps:`));
      if (options.withSql) {
        console.log(source_default.gray("   • Add your SQL files to the sql.files array"));
        console.log(source_default.gray("   • Configure SQL execution stage and error handling"));
        if (options.withMatrix) {
          console.log(source_default.gray("   • Set up matrix combinations for different SQL states"));
        }
      }
      console.log(source_default.gray("   • Run: testflow run --matrix (to test all combinations)"));
      console.log(source_default.gray("   • Run: testflow matrix list (to see available configurations)"));
    }
  } catch (error) {
    console.error(source_default.red(`❌ Error: ${error}`));
    process.exit(1);
...
var landoCmd = program2.command("lando").description("Lando environment management");
landoCmd.command("start").description("Start Lando environment").option("-c, --config <path>", "Configuration file path", "testflow.yaml").option("-p, --profile <name>", "Configuration profile to use").action(async (options) => {
  try {
    const config = await loadConfig(options.config, options.profile);
    const landoManager = new LandoManager(config.lando, config.sql, config.wordpress);
    await landoManager.setup();
    console.log(source_default.green("✅ Lando environment started"));
  } catch (error) {
    console.error(source_default.red(`❌ Error: ${error}`));
    process.exit(1);
  }
});
landoCmd.command("stop").description("Stop Lando environment").action(async () => {
  try {
    const config = await loadConfig("testflow.yaml");
    const landoManager = new LandoManager(config.lando, config.sql, config.wordpress);
    await landoManager.stop();
    console.log(source_default.green("✅ Lando environment stopped"));
  } catch (error) {
    console.error(source_default.red(`❌ Error: ${error}`));
    process.exit(1);
  }
});
landoCmd.command("destroy").description("Destroy Lando environment").action(async () => {
  try {
    const config = await loadConfig("testflow.yaml");
    const landoManager = new LandoManager(config.lando, config.sql, config.wordpress);
...
program2.command("status").description("Show TestFlow status and configuration").option("-c, --config <path>", "Configuration file path", "testflow.yaml").option("-p, --profile <name>", "Configuration profile to use").action(async (options) => {
  try {
    const config = await loadConfig(options.config, options.profile);
    console.log(source_default.blue("\uD83D\uDCCA TestFlow Configuration Status"));
    console.log(source_default.gray("═".repeat(50)));
    console.log(source_default.green("\uD83D\uDD27 Environment:"));
    console.log(source_default.gray(`   PHP: ${config.lando.php}`));
    console.log(source_default.gray(`   MySQL: ${config.lando.mysql}`));
    console.log(source_default.gray(`   WordPress: ${config.lando.wordpress}`));
    console.log(source_default.green(`
\uD83D\uDD0C Plugins:`));
    console.log(source_default.gray(`   ZIP files: ${config.plugins.zips.length}`));
    console.log(source_default.gray(`   Install path: ${config.plugins.installPath}`));
    if (config.sql) {
      console.log(source_default.green(`
\uD83D\uDDC4️  SQL Configuration:`));
      if (config.sql.files) {
        console.log(source_default.gray(`   Files: ${config.sql.files.length}`));
        config.sql.files.forEach((file) => {
          const exists = existsSync5(file);
          const status = exists ? source_default.green("✅") : source_default.red("❌");
          console.log(source_default.gray(`     ${status} ${file}`));
        });
      }
      console.log(source_default.gray(`   Execute order: ${config.sql.executeOrder || "after-wordpress"}`));
      console.log(source_default.gray(`   Continue on error: ${config.sql.continueOnError || false}`));
      console.log(source_default.gray(`   Instead of WordPress: ${config.sql.insteadOfWordPress || false}`));
      if (config.sql.searchReplace) {
        console.log(source_default.gray(`   Search & Replace: ${config.sql.searchReplace.enabled || false}`));
        if (config.sql.searchReplace.fromUrl) {
          console.log(source_default.gray(`     From: ${config.sql.searchReplace.fromUrl}`));
        }
        if (config.sql.searchReplace.toUrl) {
          console.log(source_default.gray(`     To: ${config.sql.searchReplace.toUrl}`));
        }
      }
    }
    if (config.matrix) {
...
\uD83D\uDCCB Matrix Configuration:`));
      if (config.matrix.sql_files) {
        console.log(source_default.gray(`   SQL file combinations: ${config.matrix.sql_files.length}`));
      }
      if (config.matrix.plugin_combinations) {
        console.log(source_default.gray(`   Plugin combinations: ${config.matrix.plugin_combinations.length}`));
      }
      if (config.matrix.environments) {
        console.log(source_default.gray(`   Environment configurations: ${config.matrix.environments.length}`));
      }
    }
    console.log(source_default.green(`
\uD83C\uDFAD Playwright:`));
    console.log(source_default.gray(`   Test directory: ${config.playwright.testDir}`));
    console.log(source_default.gray(`   Test patterns: ${config.playwright.patterns.join(", ")}`));
  } catch (error) {
    console.error(source_default.red(`❌ Error: ${error}`));
    process.exit(1);
  }
});
async function loadConfig(configPath, profile) {
